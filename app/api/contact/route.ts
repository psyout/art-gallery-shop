...
function getSmtpConfig() {
	...
	const port = process.env.CONTACT_SMTP_PORT ? Number(process.env.CONTACT_SMTP_PORT) : process.env.SMTP_PORT ? Number(process.env.SMTP_PORT) : undefined;
	...
	const secure = process.env.CONTACT_SMTP_SECURE === 'true' || (process.env.CONTACT_SMTP_SECURE == null && process.env.SMTP_SECURE === 'true');

	if (!host || !port || !user || !pass || !from) return null;
	return { host, port, user, pass, from, replyTo, bcc, secure };
}

const CONTACT_EMAIL = process.env.CONTACT_TO ?? 'hello@canvasandframe.co';
